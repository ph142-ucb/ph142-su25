...
```{r keep-exploring-the-data-1, tags=c("otter_assign_solution_cell")}
...
```{r keep-exploring-the-data-2, tags=c("otter_assign_solution_cell")}
...
```{r keep-exploring-the-data-3, tags=c("otter_assign_solution_cell")}
...
```{r keep-exploring-the-data-4, tags=c("otter_assign_solution_cell")}
...
- `str()`: returns information about the types of variables found in the dataset in terms of whether they are quantitative (int, num) or categorical (factor) and provides some information about each one.
...
<!-- #region tags=["otter_assign_solution_cell"] -->
The individuals are countries. There are 137 of them.
<!-- #endregion -->
\newpage 
...
# BEGIN SOLUTION
p6 <- ggplot(CS_data, aes(x = Income_Group)) + geom_bar()
p6
# END SOLUTION
```
...
# BEGIN SOLUTION
p7 <- ggplot(CS_data, aes(x = Income_Group_order)) + geom_bar()
p7
# END SOLUTION
```
...
<!-- #region tags=["otter_assign_solution_cell"] -->
Income_Group_order is preferred because the visualization is ordered 
by increasing levels of income, while using Income_Group displays the income in alphabetical order: high income is first, followed by low income, and then middle income, which is less intuitive.
<!-- #endregion -->
...
<!-- #region tags=["otter_assign_solution_cell"] -->
No, I didn't use `stat="identity"` argument. This is because I did not specify a y variable containing a count or percent for ggplot to plot on the y-axis. When `geom_bar()` is doing the counting for us, we do not use the `stat="identity"` argument.
<!-- #endregion -->
\newpage 
...
# BEGIN SOLUTION
p14 <- ggplot(CS_data, aes(x = GDP_2006)) + geom_histogram(binwidth = 2000)
p14
# END SOLUTION
```
...
# BEGIN SOLUTION 
p15 <- ggplot(CS_data, aes(x = CS_rate_100)) + geom_histogram(binwidth = 4)
p15
# END SOLUTION
```
...
```{r fig.width=11, tags=c("otter_assign_solution_cell")}
. = " # BEGIN PROMPT
...
p16 <- ggplot(CS_data, aes(x = CS_rate_100)) +
        ...
        labs(y = "Count of countries", x = "Cesarean delivery rate (%)", 
              title = "Distribution of cesarean delivery rates
             across countries") +
        theme_minimal() 
# END SOLUTION
```
...
# BEGIN SOLUTION
ggsave(filename = "hist-cs.png", plot = p16,
